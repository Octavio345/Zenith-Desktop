...
export default function CadastrarFazenda() {
  ...
  const [formData, setFormData] = useState({ name: "", tipo_proprietario: "PJ", documento_proprietario: "", data_aquisicao: "", cep: "", bairro: "", municipio: "", uf: "", area_total: "", telefone: "", plantacao: "Soja" })
  ...
  const handleRegister = async (event) => {
    ...
    if (!isValidBrazilianDocument(formData.documento_proprietario, "PJ")) { setNotice("Informe um CNPJ rural válido."); return }
    try {
      setLoading(true)
      ...
      <section className="farm-registration__form-area">
        ...
        <form className="farm-registration__card" onSubmit={handleRegister}>
          ...
          <div className="farm-registration__fields">
            ...
            <label className="farm-field"><span>CNPJ rural</span><input name="documento_proprietario" value={formData.documento_proprietario} onChange={handleChange} inputMode="numeric" maxLength={18} placeholder="00.000.000/0000-00" /><small>Inscrição do produtor rural associada ao CNPJ da fazenda.</small></label>
